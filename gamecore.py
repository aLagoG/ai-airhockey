...
class GameCore:
    def __init__(self, p1, p2, board, init_state, error_rate, gui_core):
        ...
        self.player2 = p2
        self.goal_sides = {
            "left": p1 if p1.my_goal == "left" else p2,
    ...
    def begin_game(self):
        while True:
            ...
            try:
                t0 = time.time()
                self.state["paddle1_pos"] = self.make_player_move(
                    state_copy, self.player1
                )
                assert time.time() - t0 < self.max_time, (
                ...
                return {
                    ...
                    "info": exc,
                    "goals": self.goals,
                    "winner": self.player2.my_display_name,
                }
            ...
            try:
                t0 = time.time()
                self.state["paddle2_pos"] = self.make_player_move(
                    state_copy, self.player2
                )
                assert time.time() - t0 < self.max_time, (
                ...
                return {
                    ...
                    "info": exc,
                    "goals": self.goals,
                    "winner": self.player1.my_display_name,
                }
    ...
    def check_paddle_valid_move(self, new_pos, previous_pos, state, player):
        ...
        if utils.distance_between_points(new_pos, previous_pos) > max_offset:
            raise ValueError(
                "RULES VIOLATION: paddle moved faster than speed limit for "
        ...
        if utils.is_out_of_boundaries_paddle(new_pos, state) is not None:
            raise ValueError(
                "RULES VIOLATION: Paddle moved beyond board limits for "
                + player.my_display_name
            )
        ...
        if utils.is_inside_goal_area_paddle(new_pos, state) is True:
            raise ValueError(
                "RULES VIOLATION: Paddle moved inside goal area for "
                + player.my_display_name
            )

        # check if paddle is inside player's area
        if (
            self.goal_sides["left"] is player
            and new_pos["x"] > self.board.shape[1] / 2 - self.state["puck_radius"]
        ):
            raise ValueError(
                "RULES VIOLATION: Paddle moved beyond center line for "
                + player.my_display_name
            )
        if (
            self.goal_sides["right"] is player
            and new_pos["x"] < self.board.shape[1] / 2 + self.state["puck_radius"]
        ):
            raise ValueError(
                "RULES VIOLATION: Paddle moved beyond center line for "
    ...
    def make_player_move(self, state, player):
        ...
        paddle_pos = (
            "paddle" + ("1" if self.goal_sides["left"] is player else "2") + "_pos"
        )
        self.check_paddle_valid_move(
            paddle_new_pos, self.state[paddle_pos], self.state, player
        )

        # add randomness of self.error rate maximum size and inside the board
        paddle_new_pos = {
            k: v + random.uniform(-self.error_rate, self.error_rate)
            for k, v in paddle_new_pos.items()
        }

        # rectify overlap if exists
        paddle_new_pos = utils.rectify_circles_overlap(
            self.state["puck_pos"],
        ...
        paddle_new_pos = utils.rectify_circle_out_of_bounds(
            paddle_new_pos, paddle_speed, goal_side, self.state
        )
    ...
    def set_random_position_at(self, side):
        ...
        pos = {
            "x": self.board.shape[1] * x_offset,
            "y": random.uniform(
                0 + self.state["puck_radius"],
                self.board.shape[0] - self.state["puck_radius"],
            ),
        }
        return pos
    ...
    def process_goal_for(self, goal_for, puck_to=None):
        ...
        self.state["paddle2_pos"] = {
            "x": self.board.shape[1]
            - self.board.shape[0] * self.state["goal_size"] / 2
            - 1,
            "y": self.board.shape[0] / 2,
        }
        return
    ...
    def process_move_from_state(self):
        ...
        if self.in_initial_state is not None:
            ...
            # if number of idle moves exceeded, penalize with goal
                ...
                goal_for = (
                    "left"
                    if self.state["puck_pos"]["x"] > self.board.shape[1] / 2
                    else "right"
                )
                self.process_goal_for(
                    goal_for, puck_to=("left" if goal_for == "left" else "right")
                )
                return goal_for
        ...
        # update pos in state
        self.state["puck_pos"] = utils.rectify_cicle_out_of_board(
            new_puck_pos, self.state["puck_speed"], None, self.state
        )
    ...
    def resolve_gui(self):
        flag = self.gui_core.resolve_gui(
            self.state, self.player1.my_display_name, self.player2.my_display_name
        )
        if flag < 0:
            self.gui_core.release_all()
            return {
                "status": "ERROR",
                "info": "Program exited by user (ESC key pressed)",
                "goals": self.goals,
                "winner": None,
            }
        return None
