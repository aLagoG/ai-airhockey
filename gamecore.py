...
class GameCore:
    ...
    def make_player_move(self, state, player):
        ...
        ## horizontal shift
        lower_bound = min(
            paddle_new_pos["x"] - self.state["paddle_radius"], self.error_rate
        )
        upper_bound = min(
            self.state["board_shape"][1]
            - paddle_new_pos["x"]
        ...
        ## vertical shift
        lower_bound = min(
            paddle_new_pos["y"] - self.state["paddle_radius"], self.error_rate
        )
        upper_bound = min(
            self.state["board_shape"][0]
            - paddle_new_pos["y"]
            - self.state["paddle_radius"],
            self.error_rate,
        )
        ...
        if utils.is_inside_goal_area_paddle(paddle_new_pos, self.state):
            center = {
                "x": 0 if self.goal_sides["left"] is player else self.board.shape[1],
